...
#wildboar_crop$Frucht <- str_replace(wildboar_crop$Frucht, #"Acker|Bohnen|Brache|Chinaschilf|Karotten|Kartoffeln|Kohl|Kohlrabi|Kuerbis|Mais|Mangold|Raps|Rhabarber|Roggen|Salat|Sellerie|Weide|Weizen|Wiese#|Zwiebeln", "other")


#Visually exploring the spatio-temporal patterns of wild boar in relation to the crops

#wildboar_crop$Frucht <- as.factor(wildboar_crop$Frucht)

#wildboar_Sum <- wildboar_crop %>%
#group_by(TierName, Frucht, time) %>%
#summarise(Percentage = n())
#wildboar_Sum

#wildboar_Sum %>%
#ggplot(aes(x = time, y = Percentage, fill = Frucht)) +
#geom_col() +
#scale_y_continuous(labels = scales::percent) +
#facet_wrap(~TierName)
...
wildschwein_vegetation_overlay <- terra::extract(vegetation, wildschwein)
